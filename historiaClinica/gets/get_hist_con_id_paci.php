...
$sql = "SELECT p.*,o.siglas AS obra,TIMESTAMPDIFF(YEAR, p.fecha_nac, CURDATE()) AS edad, r.nombreYapellido AS responsable, prof.nombreYapellido AS profesional,h.*
        FROM paciente p
        LEFT JOIN obra_social o ON o.id=p.obra_social
        LEFT JOIN hc_admision_ambulatorio h ON h.id_paciente = p.id
