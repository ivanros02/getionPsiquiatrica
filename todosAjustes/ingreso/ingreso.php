...
if (isset($_GET['eliminar'])) {
    ...
    $sql = "DELETE FROM tipo_ingreso WHERE id = ?";
...
if (isset($_GET['id'])) {
    ...
    $sql = "SELECT * FROM tipo_ingreso WHERE id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $id);
    $stmt->execute();
    ...
    if ($result->num_rows > 0) {
        ...
        echo "<script>editarIngreso(" . json_encode($row) . ");</script>";
    }
    $stmt->close();
}
...
<body>
    ...
    <div class="container">
        <div class="title-container">
            ...
            <button type="button" class="btn btn-custom btn-lg" data-bs-toggle="modal"
                data-bs-target="#agregarIngresoModal">
                Agregar tipo de ingreso <img src="../../img/egreso.png" alt="Icono agregar tipo de ingreso"
                ...
                <?php if ($result->num_rows > 0): ?>
                    <?php while ($row = $result->fetch_assoc()): ?>
                        <tr>  
                            ...
                            <td>
                                <button class="btn btn-custom-editar" onclick='editarIngreso(<?= json_encode($row) ?>)'><i
    ...
    <div class="modal fade" id="agregarIngresoModal" tabindex="-1" aria-labelledby="exampleModalLabel"
        aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Agregar tipo de Ingreso</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form id="formIngreso" action="./agregarIngreso.php" method="POST">
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function () {
            ...
            if (urlParams.has('success') && urlParams.get('success') === 'true') {
                alert("El tipo de ingreso se ha editado correctamente.");
        ...
        document.addEventListener("DOMContentLoaded", function() {
            function editarIngreso(ingreso) {
                document.getElementById('formIngreso').action = './editarIngreso.php'; // Asegúrate de que el formulario apunta a la URL correcta

                document.getElementById('id').value = ingreso.id;
                document.getElementById('codigo').value = ingreso.codigo;
                document.getElementById('descripcion').value = ingreso.descripcion;

                // Mostrar el modal de edición
                var modal = new bootstrap.Modal(document.getElementById('agregarIngresoModal'));
                modal.show();
            }

            // Función para limpiar el formulario
            function limpiarFormulario() {
                document.getElementById('formIngreso').action = './agregarIngreso.php';
            ...
            var btnAgregarIngresoModal = document.querySelector('button[data-bs-target="#agregarIngresoModal"]');
            btnAgregarIngresoModal.addEventListener('click', limpiarFormulario);
