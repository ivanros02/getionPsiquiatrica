...
if (isset($_GET['eliminar'])) {
    ...
    $sql = "DELETE FROM tipo_afiliado WHERE id = ?";
    ...
    try {
        ...
        if ($stmt->execute()) {
            ...
            header("Location: ./tipoAfiliado.php");
            exit();
        } else {
        ...
        if ($e->getCode() == 1451) {
            ...
            echo "<script>
                    alert('Error al eliminar, tipo de afiliado relacionada a un paciente');
                    window.location.href = './tipoAfiliado.php';
                  </script>";
        } else {
            // Otro error
...
if (isset($_GET['id'])) {
    ...
    $sql = "SELECT * FROM tipo_afiliado WHERE id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $id);
    $stmt->execute();
    ...
    if ($result->num_rows > 0) {
        ...
        echo "<script>editarTipo(" . json_encode($row) . ");</script>";
    }
    $stmt->close();
}

// Obtener todas las especialidades
$sql = "SELECT * FROM tipo_afiliado";
...
<body>
    ...
    <div class="container">
        <div class="title-container">
            ...
            <h2>Tipo de afiliados</h2>
            <!-- Botón para agregar profesional -->
            <button type="button" class="btn btn-custom btn-lg" data-bs-toggle="modal"
                data-bs-target="#agregarTipoModal">
                Agregar Tipo de afiliado <img src="../../img/tipoAfiliado.png" alt="Icono agregar tipoAfiliado"
                ...
                <?php if ($result->num_rows > 0): ?>
                    <?php while ($row = $result->fetch_assoc()): ?>
                        <tr>  
                            ...
                            <td>
                                <button class="btn btn-custom-editar" onclick='editarTipo(<?= json_encode($row) ?>)'><i
                                        class="fas fa-pencil-alt"></i></button>


                                <a href="?eliminar=<?= $row['id'] ?>" class="btn btn-danger"
                                    onclick="return confirm('¿Estás seguro de que deseas eliminar este tipo?');"><i
    ...
    <div class="modal fade" id="agregarTipoModal" tabindex="-1" aria-labelledby="exampleModalLabel"
        aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Agregar tipo de afiliado</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form id="formTipo" action="./agregarTipo.php" method="POST">
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function () {
            ...
            if (urlParams.has('success') && urlParams.get('success') === 'true') {
                alert("El tipo se ha editado correctamente.");
        ...
        document.addEventListener("DOMContentLoaded", function() {
            function editarTipo(tipo) {
                document.getElementById('formTipo').action = './editarTipo.php'; // Asegúrate de que el formulario apunta a la URL correcta
                ...
                var modal = new bootstrap.Modal(document.getElementById('agregarTipoModal'));
                modal.show();
            }

            // Función para limpiar el formulario
            function limpiarFormulario() {
                document.getElementById('formTipo').action = './agregarTipo.php';
            ...
            var btnAgregarTipoModal= document.querySelector('button[data-bs-target="#agregarTipoModal"]');
            btnAgregarTipoModal.addEventListener('click', limpiarFormulario);
