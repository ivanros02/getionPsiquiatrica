...
if (isset($_GET['eliminar'])) {
    ...
    try {
        ...
        if ($e->getCode() == 1451) {
            ...
            echo "<script>
                    alert('Error al eliminar, habitacion relacionada a un paciente');
...
if (isset($_GET['id'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        echo "<script>editarHabitacion(" . json_encode($row) . ");</script>";
    }
    $stmt->close();
}
...
<body>
    ...
    <div class="container">
        <div class="title-container">
            ...
            <button type="button" class="btn btn-custom btn-lg" data-bs-toggle="modal"
                data-bs-target="#agregarHabitacionModal">
                Agregar Habitacion <img src="../../img/habitaciones.png" alt="Icono agregar habitaciones"
                ...
                <?php if ($result->num_rows > 0): ?>
                    <?php while ($row = $result->fetch_assoc()): ?>
                        <tr>  
                            ...
                            <td><?= $row["c_camas"] ?></td>
                            <td>
                                <button class="btn btn-custom-editar" onclick='editarHabitacion(<?= json_encode($row) ?>)'><i
                                        class="fas fa-pencil-alt"></i></button>


                                <a href="?eliminar=<?= $row['id'] ?>" class="btn btn-danger"
                                    onclick="return confirm('¿Estás seguro de que deseas eliminar esta Habitacion?');"><i
    ...
    <div class="modal fade" id="agregarHabitacionModal" tabindex="-1" aria-labelledby="exampleModalLabel"
        aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="formHabitacion" action="./agregarHabitacion.php" method="POST">
                    <input type="hidden" id="id" name="id">
                        ...
                        <div class="form-group">
                            <label for="piso">Piso/Sala</label>
                            <input type="text" class="form-control" id="piso" name="piso"
                                required>
                        </div>
                        <div class="form-group">
                            <label for="c_camas">Cant. de camas</label>
                            <input type="text" class="form-control" id="c_camas" name="c_camas"
                                required>
                        </div>
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function () {
            ...
            if (urlParams.has('success') && urlParams.get('success') === 'true') {
                alert("La Habitacion se ha editado correctamente.");
        ...
        document.addEventListener("DOMContentLoaded", function() {
            function editarHabitacion(habitacion) {
                document.getElementById('formHabitacion').action = './editarHabitacion.php'; // Asegúrate de que el formulario apunta a la URL correcta

                document.getElementById('id').value = habitacion.id;
                document.getElementById('num').value = habitacion.num;
                document.getElementById('piso').value = habitacion.piso;
                document.getElementById('c_camas').value = habitacion.c_camas;

                // Mostrar el modal de edición
                var modal = new bootstrap.Modal(document.getElementById('agregarHabitacionModal'));
                modal.show();
            }

            // Función para limpiar el formulario
            function limpiarFormulario() {
                document.getElementById('formHabitacion').action = './agregarHabitacion.php';
                document.getElementById('num').value = '';
                document.getElementById('piso').value = '';
                document.getElementById('c_camas').value = '';
            }
            ...
            var btnAgregarHabitacionModal= document.querySelector('button[data-bs-target="#agregarHabitacionModal"]');
            btnAgregarHabitacionModal.addEventListener('click', limpiarFormulario);
