...
if (isset($_GET['eliminar'])) {
    ...
    $sql = "DELETE FROM obra_social WHERE id = ?";
    ...
    try {
        ...
        if ($e->getCode() == 1451) {
            ...
            echo "<script>
                    alert('Error al eliminar, obra social relacionada a un paciente');
                    window.location.href = './obraSocial.php';
                  </script>";
        } else {
            // Otro error
...
if (isset($_GET['id'])) {
    ...
    $sql = "SELECT * FROM obra_social WHERE id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $id);
    $stmt->execute();
    ...
    if ($result->num_rows > 0) {
        ...
        echo "<script>editarObraSocial(" . json_encode($row) . ");</script>";
    }
    $stmt->close();
}
...
<body>
    ...
    <div class="container">
        <div class="title-container">
            ...
            <button type="button" class="btn btn-custom btn-lg" data-bs-toggle="modal"
                data-bs-target="#agregarObraSocialModal">
                Agregar Obra Social <img src="../../img/obraSocial.png" alt="Icono agregar obra social"
    ...
    <div class="modal fade" id="agregarObraSocialModal" tabindex="-1" aria-labelledby="exampleModalLabel"
        aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Agregar Obra social</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form id="formObraSocial" action="./agregarObraSocial.php" method="POST">
    ...
    <script>
        ...
        function editarEspecialidad(especialidad) {
            document.getElementById('formObraSocial').action = './editarObraSocial.php'; // Asegúrate de que el formulario apunta a la URL correcta

            // Rellenar los campos del formulario con los datos de la especialidad
            document.getElementById('id').value = especialidad.id;
            document.getElementById('siglas').value = especialidad.siglas;
            document.getElementById('razon_social').value = especialidad.razon_social;

            // Mostrar el modal de edición
            var modal = new bootstrap.Modal(document.getElementById('agregarObraSocialModal'));
            modal.show();
        }
    </script>
