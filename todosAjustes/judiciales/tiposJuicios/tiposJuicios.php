...
if (isset($_GET['eliminar'])) {
    ...
    $sql = "DELETE FROM t_juicio WHERE id = ?";
    ...
    try {
        ...
        if ($stmt->execute()) {
            ...
            header("Location: ./tipoJuicios.php");
            exit();
        } else {
        ...
        if ($e->getCode() == 1451) {
            ...
            echo "<script>
                    alert('Error al eliminar, categoria relacionada a un paciente');
                    window.location.href = './tipoJuicios.php';
                  </script>";
        } else {
            // Otro error
...
if (isset($_GET['id'])) {
    ...
    $sql = "SELECT * FROM t_juicio WHERE id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $id);
    $stmt->execute();
    ...
    if ($result->num_rows > 0) {
        ...
        echo "<script>editarJuicio(" . json_encode($row) . ");</script>";
    }
    $stmt->close();
}

// Obtener todas las especialidades
$sql = "SELECT * FROM t_juicio";
...
<body>
    ...
    <div class="container">
        <div class="title-container">
            ...
            <h2>Tipo de juicios</h2>
            <!-- Botón para agregar profesional -->
            <button type="button" class="btn btn-custom btn-lg" data-bs-toggle="modal"
                data-bs-target="#agregarJuicioModal">
                Agregar Tipo de juicio <img src="../../../img/judiciales.png" alt="Icono agregar Tipo de juicio"
                ...
                <?php if ($result->num_rows > 0): ?>
                    <?php while ($row = $result->fetch_assoc()): ?>
                        <tr>  
                            ...
                            <td>
                                <button class="btn btn-custom-editar" onclick='editarJuicio(<?= json_encode($row) ?>)'><i
                                        class="fas fa-pencil-alt"></i></button>


                                <a href="?eliminar=<?= $row['id'] ?>" class="btn btn-danger"
                                    onclick="return confirm('¿Estás seguro de que deseas eliminar este tipo de juicio?');"><i
    ...
    <div class="modal fade" id="agregarJuicioModal" tabindex="-1" aria-labelledby="exampleModalLabel"
        aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Agregar tipo de  juicio</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form id="formJuicio" action="./agregarJuicio.php" method="POST">
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function () {
            ...
            if (urlParams.has('success') && urlParams.get('success') === 'true') {
                alert("El tipo de juicio se ha editado correctamente.");
        ...
        document.addEventListener("DOMContentLoaded", function() {
            function editarJuicio(juicio) {
                document.getElementById('formJuicio').action = './editarJuicio.php'; // Asegúrate de que el formulario apunta a la URL correcta

                document.getElementById('id').value = juicio.id;
                document.getElementById('codigo').value = juicio.codigo;
                document.getElementById('descripcion').value = juicio.descripcion;

                // Mostrar el modal de edición
                var modal = new bootstrap.Modal(document.getElementById('agregarJuicioModal'));
                modal.show();
            }

            // Función para limpiar el formulario
            function limpiarFormulario() {
                document.getElementById('formJuicio').action = './agregarJuicio.php';
            ...
            window.editarJuicio = editarJuicio;

            // Limpiar el formulario al abrir el modal para agregar profesional
            var btnAgregarJuicioModal= document.querySelector('button[data-bs-target="#agregarJuicioModal"]');
            btnAgregarJuicioModal.addEventListener('click', limpiarFormulario);
