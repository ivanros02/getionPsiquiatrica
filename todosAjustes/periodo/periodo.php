...
if (isset($_GET['eliminar'])) {
    ...
    try {
        ...
        if ($e->getCode() == 1451) {
            ...
            echo "<script>
                    alert('Error al eliminar, periodo relacionado a un gasto');
...
if (isset($_GET['id'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        echo "<script>editarPeriodo(" . json_encode($row) . ");</script>";
    }
    $stmt->close();
}
...
<body>
    ...
    <div class="container">
        <div class="title-container">
            ...
            <button type="button" class="btn btn-custom btn-lg" data-bs-toggle="modal"
                data-bs-target="#agregarPeriodoModal">
                Agregar Periodo <img src="../../img/periodo.png" alt="Icono agregar periodo"
                ...
                <?php if ($result->num_rows > 0): ?>
                    <?php while ($row = $result->fetch_assoc()): ?>
                        <tr>  
                            ...
                            <td>
                                <button class="btn btn-custom-editar" onclick='editarPeriodo(<?= json_encode($row) ?>)'><i
                                        class="fas fa-pencil-alt"></i></button>


                                <a href="?eliminar=<?= $row['id'] ?>" class="btn btn-danger"
                                    onclick="return confirm('¿Estás seguro de que deseas eliminar este periodo?');"><i
    ...
    <div class="modal fade" id="agregarPeriodoModal" tabindex="-1" aria-labelledby="exampleModalLabel"
        aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="formPeriodo" action="./agregarPeriodo.php" method="POST">
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function () {
            ...
            if (urlParams.has('success') && urlParams.get('success') === 'true') {
                alert("El Periodo se ha editado correctamente.");
        ...
        document.addEventListener("DOMContentLoaded", function() {
            function editarPeriodo(periodo) {
                document.getElementById('formPeriodo').action = './editarPeriodo.php'; // Asegúrate de que el formulario apunta a la URL correcta

                document.getElementById('id').value = periodo.id;
                document.getElementById('codigo').value = periodo.codigo;
                document.getElementById('descripcion').value = periodo.descripcion;

                // Mostrar el modal de edición
                var modal = new bootstrap.Modal(document.getElementById('agregarPeriodoModal'));
                modal.show();
            }

            // Función para limpiar el formulario
            function limpiarFormulario() {
                document.getElementById('formPeriodo').action = './agregarPeriodo.php';
            ...
            var btnAgregarPeriodoModal= document.querySelector('button[data-bs-target="#agregarPeriodoModal"]');
            btnAgregarPeriodoModal.addEventListener('click', limpiarFormulario);
