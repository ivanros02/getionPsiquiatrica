...
if (isset($_GET['eliminar'])) {
    ...
    $sql = "DELETE FROM tipo_egreso WHERE id = ?";
...
if (isset($_GET['id'])) {
    ...
    $sql = "SELECT * FROM tipo_egreso WHERE id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $id);
    $stmt->execute();
    ...
    if ($result->num_rows > 0) {
        ...
        echo "<script>editarEgreso(" . json_encode($row) . ");</script>";
    }
    $stmt->close();
}
...
<body>
    ...
    <div class="container">
        <div class="title-container">
            ...
            <button type="button" class="btn btn-custom btn-lg" data-bs-toggle="modal"
                data-bs-target="#agregarEgresoModal">
                Agregar tipo de egreso <img src="../img/egreso.png" alt="Icono agregar tipo de egreso"
                ...
                <?php if ($result->num_rows > 0): ?>
                    <?php while ($row = $result->fetch_assoc()): ?>
                        <tr>  
                            ...
                            <td>
                                <button class="btn btn-custom-editar" onclick='editarEgreso(<?= json_encode($row) ?>)'><i
    ...
    <div class="modal fade" id="agregarEgresoModal" tabindex="-1" aria-labelledby="exampleModalLabel"
        aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="formEgreso" action="./agregarEgreso.php" method="POST">
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function () {
            ...
            if (urlParams.has('success') && urlParams.get('success') === 'true') {
                alert("El tipo de egreso se ha editado correctamente.");
        ...
        document.addEventListener("DOMContentLoaded", function() {
            function editarEgreso(egreso) {
                document.getElementById('formEgreso').action = './editarEgreso.php'; // Asegúrate de que el formulario apunta a la URL correcta

                document.getElementById('id').value = egreso.id;
                document.getElementById('codigo').value = egreso.codigo;
                document.getElementById('descripcion').value = egreso.descripcion;

                // Mostrar el modal de edición
                var modal = new bootstrap.Modal(document.getElementById('agregarEgresoModal'));
                modal.show();
            }

            // Función para limpiar el formulario
            function limpiarFormulario() {
                document.getElementById('formEgreso').action = './agregarEgreso.php';
            ...
            var btnAgregarEgresoModal = document.querySelector('button[data-bs-target="#agregarEgresoModal"]');
