...
if (isset($_GET['eliminar'])) {
    ...
    try {
        ...
        if ($e->getCode() == 1451) {
            ...
            echo "<script>
                    alert('Error al eliminar, secretaria relacionada a un paciente');
                    window.location.href = './secretarias.php';
                  </script>";
        } else {
            // Otro error
...
if (isset($_GET['id'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        echo "<script>editarSecretaria(" . json_encode($row) . ");</script>";
    }
    $stmt->close();
}
...
<body>
    ...
    <div class="container">
        <div class="title-container">
            ...
            <button type="button" class="btn btn-custom btn-lg" data-bs-toggle="modal"
                data-bs-target="#agregarSecretariaModal">
                Agregar Secretaria <img src="../../../img/judiciales.png" alt="Icono agregar Secretaria"
                ...
                <?php if ($result->num_rows > 0): ?>
                    <?php while ($row = $result->fetch_assoc()): ?>
                        <tr>
                            ...
                            <td><?= $row["tribunal"] ?></td>
                            <td>
                                <button class="btn btn-custom-editar" onclick='editarSecretaria(<?= json_encode($row) ?>)'><i
                                        class="fas fa-pencil-alt"></i></button>
                                <a href="?eliminar=<?= $row['id'] ?>" class="btn btn-danger"
                                    onclick="return confirm('¿Estás seguro de que deseas eliminar esta secretaria?');"><i
    ...
    <div class="modal fade" id="agregarSecretariaModal" tabindex="-1" aria-labelledby="exampleModalLabel"
        aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Agregar Secretaria</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form id="formSecretaria" action="./agregarSecretaria.php" method="POST">
                        <input type="hidden" id="id" name="id">
                        <div class="form-group">
                            <label for="nro_secretaria">Nro de secretaria</label>
                            <input type="text" class="form-control" id="nro_secretaria" name="nro_secretaria" required>
                        </div>
                        <div class="form-group">
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function () {
            ...
            if (urlParams.has('success') && urlParams.get('success') === 'true') {
                alert("La Secretaria se ha editado correctamente.");
        ...
        document.addEventListener("DOMContentLoaded", function () {
            function editarSecretaria(secretaria) {
                document.getElementById('formSecretaria').action = './editarSecretaria.php'; // Asegúrate de que el formulario apunta a la URL correcta

                document.getElementById('id').value = secretaria.id;
                document.getElementById('nro_secretaria').value = secretaria.nro_secretaria;
                document.getElementById('descripcion').value = secretaria.descripcion;
                document.getElementById('calle').value = secretaria.calle;
                document.getElementById('numero').value = secretaria.numero;
                document.getElementById('localidad').value = secretaria.localidad;
                document.getElementById('provincia').value = secretaria.provincia;
                document.getElementById('c_postal').value = secretaria.c_postal;
                document.getElementById('tribunal').value = secretaria.tribunal;

                // Mostrar el modal de edición
                var modal = new bootstrap.Modal(document.getElementById('agregarSecretariaModal'));
                modal.show();
            }

            // Función para limpiar el formulario
            function limpiarFormulario() {
                document.getElementById('formSecretaria').action = './agregarSecretaria.php';
                document.getElementById('id').value = '';
                document.getElementById('nro_secretaria').value = '';
            ...
            window.editarSecretaria = editarSecretaria;

            // Limpiar el formulario al abrir el modal para agregar juzgado
            var btnAgregarSecretariaModal = document.querySelector('button[data-bs-target="#agregarSecretariaModal"]');
            btnAgregarSecretariaModal.addEventListener('click', limpiarFormulario);
        });
