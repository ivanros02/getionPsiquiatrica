...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "INSERT INTO tipo_ingreso (codigo, descripcion) VALUES (?, ?)";
    ...
    if ($stmt->execute()) {
        ...
        echo "<script>alert('Tipo de ingreso agregado correctamente.'); window.location.href = './ingreso.php';</script>";
    } else {
        // Si hubo un error al agregar la especialidad, mostrar una alerta con el mensaje de error
        echo "<script>alert('Error al agregar el ingreso: " . $stmt->error . "');</script>";
    }
