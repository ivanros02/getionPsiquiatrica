...
if (isset($_GET['eliminar'])) {
    ...
    try {
        ...
        if ($e->getCode() == 1451) {
            ...
            echo "<script>
                    alert('Error al eliminar,modalidad relacionada a un paciente');
                    window.location.href = './modalidad.php';
                  </script>";
        } else {
            // Otro error
            echo "Error al eliminar la modalidad: " . $e->getMessage();
        }
    }
...
if (isset($_GET['id'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        echo "<script>editarModalidad(" . json_encode($row) . ");</script>";
    }
    $stmt->close();
}
...
<body>
    ...
    <div class="container">
        <div class="title-container">
            ...
            <button type="button" class="btn btn-custom btn-lg" data-bs-toggle="modal"
                data-bs-target="#agregarModalidadModal">
                Agregar Modalidad <img src="../../img/modalidad.png" alt="Icono agregar modalidad"
                ...
                <?php if ($result->num_rows > 0): ?>
                    <?php while ($row = $result->fetch_assoc()): ?>
                        <tr>  
                            ...
                            <td>
                                <button class="btn btn-custom-editar" onclick='editarModalidad(<?= json_encode($row) ?>)'><i
                                        class="fas fa-pencil-alt"></i></button>


                                <a href="?eliminar=<?= $row['id'] ?>" class="btn btn-danger"
                                    onclick="return confirm('¿Estás seguro de que deseas eliminar esta Modalidad?');"><i
    ...
    <div class="modal fade" id="agregarModalidadModal" tabindex="-1" aria-labelledby="exampleModalLabel"
        aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="formModalidad" action="./agregarModalidad.php" method="POST">
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function () {
            ...
            if (urlParams.has('success') && urlParams.get('success') === 'true') {
                alert("La Modalidad se ha editado correctamente.");
        ...
        document.addEventListener("DOMContentLoaded", function() {
            function editarModalidad(modalidad) {
                document.getElementById('formModalidad').action = './editarModalidad.php'; // Asegúrate de que el formulario apunta a la URL correcta

                document.getElementById('id').value = modalidad.id;
                document.getElementById('codigo').value = modalidad.codigo;
                document.getElementById('descripcion').value = modalidad.descripcion;

                // Mostrar el modal de edición
                var modal = new bootstrap.Modal(document.getElementById('agregarModalidadModal'));
                modal.show();
            }

            // Función para limpiar el formulario
            function limpiarFormulario() {
                document.getElementById('formModalidad').action = './agregarModalidad.php';
            ...
            window.editarModalidad = editarModalidad;

            // Limpiar el formulario al abrir el modal para agregar profesional
            var btnAgregarModalidadModal= document.querySelector('button[data-bs-target="#agregarModalidadModal"]');
            btnAgregarModalidadModal.addEventListener('click', limpiarFormulario);
