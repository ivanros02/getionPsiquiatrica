...
if (isset($_GET['eliminar'])) {
    ...
    $sql = "DELETE FROM comprobantes WHERE id = ?";
    ...
    try {
        ...
        if ($e->getCode() == 1451) {
            ...
            echo "<script>
                    alert('Error al eliminar, comprobante relacionado a un gasto');
...
if (isset($_GET['id'])) {
    ...
    $sql = "SELECT * FROM comprobantes WHERE id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $id);
    $stmt->execute();
    ...
    if ($result->num_rows > 0) {
        ...
        echo "<script>editarComprobante(" . json_encode($row) . ");</script>";
    }
    $stmt->close();
}
...
<body>
    ...
    <div class="container">
        <div class="title-container">
            ...
            <button type="button" class="btn btn-custom btn-lg" data-bs-toggle="modal"
                data-bs-target="#agregarComprobanteModal">
                Agregar Comprobante <img src="../../img/comprobante.png" alt="Icono agregar comprobantes"
                        ...
                        <tr>  
                            ...
                            <td>
                                ...
                                <a href="?eliminar=<?= $row['id'] ?>" class="btn btn-danger"
                                    onclick="return confirm('¿Estás seguro de que deseas eliminar esta Comprobante?');"><i
    ...
    <div class="modal fade" id="agregarComprobanteModal" tabindex="-1" aria-labelledby="exampleModalLabel"
        aria-hidden="true">
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function () {
            ...
            if (urlParams.has('success') && urlParams.get('success') === 'true') {
                alert("El Comprobante se ha editado correctamente.");
        ...
        document.addEventListener("DOMContentLoaded", function() {
            function editarComprobante(comprobante) {
                ...
                var modal = new bootstrap.Modal(document.getElementById('agregarComprobanteModal'));
                modal.show();
            }
            ...
            window.editarComprobante = editarComprobante;

            // Limpiar el formulario al abrir el modal para agregar profesional
            var btnAgregarComprobanteModal= document.querySelector('button[data-bs-target="#agregarComprobanteModal"]');
            btnAgregarComprobanteModal.addEventListener('click', limpiarFormulario);
