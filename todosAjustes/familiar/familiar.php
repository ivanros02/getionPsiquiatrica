...
if (isset($_GET['eliminar'])) {
    ...
    $sql = "DELETE FROM tipo_familiar WHERE id = ?";
    ...
    try {
        ...
        if ($e->getCode() == 1451) {
            ...
            echo "<script>
                    alert('Error al eliminar, familiar relacionada a un paciente');
                    window.location.href = './familiar.php';
                  </script>";
        } else {
            // Otro error
...
if (isset($_GET['id'])) {
    ...
    $sql = "SELECT * FROM tipo_familiar WHERE id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $id);
    $stmt->execute();
    ...
    if ($result->num_rows > 0) {
        ...
        echo "<script>editarFamiliar(" . json_encode($row) . ");</script>";
    }
    $stmt->close();
}

// Obtener todas las especialidades
$sql = "SELECT * FROM tipo_familiar";
...
<body>
    ...
    <div class="container">
        <div class="title-container">
            ...
            <button type="button" class="btn btn-custom btn-lg" data-bs-toggle="modal"
                data-bs-target="#agregarFamiliarModal">
                Agregar Familiar <img src="../../img/familiar.png" alt="Icono agregar Familiar"
                ...
                <?php if ($result->num_rows > 0): ?>
                    <?php while ($row = $result->fetch_assoc()): ?>
                        <tr>  
                            ...
                            <td>
                                <button class="btn btn-custom-editar" onclick='editarFamiliar(<?= json_encode($row) ?>)'><i
                                        class="fas fa-pencil-alt"></i></button>


                                <a href="?eliminar=<?= $row['id'] ?>" class="btn btn-danger"
                                    onclick="return confirm('¿Estás seguro de que deseas eliminar este familiar?');"><i
    ...
    <div class="modal fade" id="agregarFamiliarModal" tabindex="-1" aria-labelledby="exampleModalLabel"
        aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="formFamiliar" action="./agregarFamiliar.php" method="POST">
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function () {
            ...
            if (urlParams.has('success') && urlParams.get('success') === 'true') {
                alert("El Familiar se ha editado correctamente.");
        ...
        document.addEventListener("DOMContentLoaded", function() {
            function editarFamiliar(familiar) {
                document.getElementById('formFamiliar').action = './editarFamiliar.php'; // Asegúrate de que el formulario apunta a la URL correcta

                document.getElementById('id').value = familiar.id;
                document.getElementById('codigo').value = familiar.codigo;
                document.getElementById('descripcion').value = familiar.descripcion;

                // Mostrar el modal de edición
                var modal = new bootstrap.Modal(document.getElementById('agregarFamiliarModal'));
                modal.show();
            }

            // Función para limpiar el formulario
            function limpiarFormulario() {
                document.getElementById('formFamiliar').action = './agregarFamiliar.php';
            ...
            window.editarFamiliar = editarFamiliar;

            // Limpiar el formulario al abrir el modal para agregar profesional
            var btnAgregarFamiliarModal= document.querySelector('button[data-bs-target="#agregarFamiliarModal"]');
            btnAgregarFamiliarModal.addEventListener('click', limpiarFormulario);
