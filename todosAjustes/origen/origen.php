...
if (isset($_GET['eliminar'])) {
    ...
    $sql = "DELETE FROM origen_ingreso WHERE id = ?";
    ...
    try {
        ...
        if ($e->getCode() == 1451) {
            ...
            echo "<script>
                    alert('Error al eliminar, origen relacionada a un paciente');
                    window.location.href = './ingreso.php';
                  </script>";
        } else {
            // Otro error
            echo "Error al eliminar el origen de ingreso: " . $e->getMessage();
        }
    }
...
if (isset($_GET['id'])) {
    ...
    $sql = "SELECT * FROM origen_ingreso WHERE id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $id);
    $stmt->execute();
    ...
    if ($result->num_rows > 0) {
        ...
        echo "<script>editarOrigen(" . json_encode($row) . ");</script>";
    }
    $stmt->close();
}

// Obtener todas las especialidades
$sql = "SELECT * FROM origen_ingreso";
...
<body>
    ...
    <div class="container">
        <div class="title-container">
            ...
            <h2>Origen de ingreso</h2>
            <!-- Botón para agregar profesional -->
            <button type="button" class="btn btn-custom btn-lg" data-bs-toggle="modal"
                data-bs-target="#agregarOrigenModal">
                Agregar origen de ingreso <img src="../../img/origen.png" alt="Icono agregar origen de ingreso"
                ...
                <?php if ($result->num_rows > 0): ?>
                    <?php while ($row = $result->fetch_assoc()): ?>
                        <tr>  
                            ...
                            <td>
                                <button class="btn btn-custom-editar" onclick='editarOrigen(<?= json_encode($row) ?>)'><i
                                        class="fas fa-pencil-alt"></i></button>


                                <a href="?eliminar=<?= $row['id'] ?>" class="btn btn-danger"
                                    onclick="return confirm('¿Estás seguro de que deseas eliminar este tipo de egreso?');"><i
    ...
    <div class="modal fade" id="agregarOrigenModal" tabindex="-1" aria-labelledby="exampleModalLabel"
        aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Agregar origen de ingreso</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form id="formOrigen" action="./agregarOrigen.php" method="POST">
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function () {
            ...
            if (urlParams.has('success') && urlParams.get('success') === 'true') {
                alert("El origen de ingreso se ha editado correctamente.");
        ...
        document.addEventListener("DOMContentLoaded", function() {
            function editarOrigen(origen) {
                document.getElementById('formOrigen').action = './editarOrigen.php'; // Asegúrate de que el formulario apunta a la URL correcta

                document.getElementById('id').value = origen.id;
                document.getElementById('codigo').value = origen.codigo;
                document.getElementById('descripcion').value = origen.descripcion;

                // Mostrar el modal de edición
                var modal = new bootstrap.Modal(document.getElementById('agregarOrigenModal'));
                modal.show();
            }

            // Función para limpiar el formulario
            function limpiarFormulario() {
                document.getElementById('formOrigen').action = './agregarOrigen.php';
            ...
            window.editarOrigen = editarOrigen;

            // Limpiar el formulario al abrir el modal para agregar profesional
            var btnAgregarOrigenModal = document.querySelector('button[data-bs-target="#agregarOrigenModal"]');
            btnAgregarOrigenModal.addEventListener('click', limpiarFormulario);
